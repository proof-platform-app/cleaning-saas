...
– Mobile Cleaner App: login, Today Jobs, Job Details, check-in / check-out,
  загрузка before/after фото — работают на реальном API.
– Чек-лист на мобильном пока **только read-only** (изменения будем
  вводить отдельной фазой).
...
Кнопки Upload Photo / Check-out можно:

либо скрыть,

либо показать в disabled (на выбор, логика всё равно на backend)
...
Условия на backend (для понимания фронта):
...
показывает пользователю ошибку вида: "Can't get location, please try again"
...
Check-out — активна всегда при status="in_progress".
Backend всё равно проверит чеклист и фото.
...
Check-in / Upload / Check-out → скрыты или disabled (на выбор)
...
фронтенд отправляет изменения на backend через существующий endpoint bulk-update чеклиста
...
При ошибке (например, job уже completed) → показываем detail, откатываем локальное изменение.
