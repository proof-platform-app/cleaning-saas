...
## 0. Текущий статус проекта (Phase 9)
...
– Job Details  
– check-in / check-out  
– загрузка before / after фото  

→ работают на реальном API.

**Ограничение**
– чек-лист на мобильном пока **только read-only**  
– изменения чек-листа будут вводиться отдельной фазой.
...
## 8. Location input & map behavior (Intentional design)
...
* по факту ограничения выражаются через UI;
* жёсткие блокировки сознательно не используются до биллинга.
...
* загрузка логотипа компании реализована в Company settings,  
  но рендер логотипа в PDF пока сознательно отложен.
...
A dedicated endpoint allows managers to navigate from aggregated SLA metrics  
(weekly / monthly reports) to the exact jobs that caused a specific SLA violation.

The frontend exposes this via a conditional screen (`/reports/violations`)  
that is accessed only through contextual navigation  
(reason + period), not via the main menu.

Important:

The mobile cleaner app currently enforces mandatory photos and checklist completion before job checkout.  
...
## 39. Photos — Mobile + Backend DONE (Phase 9)
...
* после создания:
  ...
  * закрыть drawer;
  * опционально открыть side panel.

### Шаг 3 — Planning фильтры “как в Lovable”

* фильтр по `cleanerIds`;
* фильтр по `locationId`;
* фильтр statuses — довести до полного соответствия.

### Шаг 4 — Визуальная полировка

* StatusPill и Proof-иконки привести к одному стилю;
...
## 56. С ЧЕГО НАЧАТЬ НОВЫЙ ЧАТ (ОБЯЗАТЕЛЬНО СКОПИРОВАТЬ)

В новом чате первый месседж должен быть вот таким:
...
* PDF job report — отправка в один клик;
...
* история отображается в Job Details.

Ограничение:
* email history — только в контексте job;
* агрегация вынесена в следующий этап.
...
Frontend:
* не выводит это как ошибку;
* отображает read-only режим;
* даёт объясняющий UX.
...
* не ломают рабочий процесс;
* не создают стресс.

Технические блокировки — только по решению backend’а.
...
- **Completed** — only recently completed jobs (default: last 30 days).

The Jobs page intentionally does **not** show the full historical dataset to avoid
overloaded UI and unnecessary data transfer.

The **Job History** page is the authoritative archive:
- supports arbitrary date ranges,
- advanced filters,
- and access to the full job history without time limits.

**Reports UI — structural decisions**

Reports page is intentionally split into two conceptual layers:

* **Owner view** — read-only, high-level summary with no drill-down actions.
* **Manager view** — actionable layer with navigation to violation details and job history.

Owner view:

* Shows a rolling 30-day window.
* Contains only aggregated KPIs and top entities.
* Does not allow navigation to Job History or individual jobs.

Manager view:

* Displays owner summary first for shared context.
* Focuses on actionable blocks in fixed order:

  1. Period summary
  2. Top SLA reasons (root causes)
  3. Cleaners with violations
  4. Locations with violations

Layout decisions:

* Main content container is aligned closer to the sidebar to avoid unused horizontal space.
* Reports are visually wider than standard forms to emphasize analytical reading over data entry.
...
Вкладка Email history (/reports/email-logs) приведена к тому же уровню строгости, что и Reports: единый журнал всех job/weekly/monthly писем с фильтрами и статусами.

Layout
В AppLayout увеличена максимальная ширина контента, чтобы таблицы Reports и Email history визуально доходили до сайдбара и не оставляли «пустого поля».
...
Завершена разработка backend-логики аналитики и SLA. Вынесены переиспользуемые расчёты summary-метрик и дельт по периодам. Все edge-cases (пустые периоды, деление на ноль, отсутствие чеклистов или фото) обработаны на уровне API. Frontend получает уже нормализованные данные и не содержит бизнес-логики расчётов. Аналитика строится строго на completed jobs, с использованием actual_start_time / actual_end_time и существующего SLA engine.
