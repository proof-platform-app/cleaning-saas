...
Summary-метрики с дельтами по периодам
Тренды: jobs completed, duration, proof completion, SLA violations
SLA Performance: overview, violation reasons, hotspots (cleaners / locations)
Performance breakdown по клинерам
Все метрики считаются только по completed jobs
API стабилен, без null / invalid значений
Frontend использует API как source of truth, без расчётов на клиенте
