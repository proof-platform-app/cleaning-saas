...
# =============================================================================
# Maintenance Report PDF (S2-P3)
# =============================================================================

def generate_maintenance_report_pdf(company, report_data: dict, frequency: str = "Weekly") -> bytes:
    """
    PDF report for maintenance context.
    Uses neutral gray/slate color scheme (different from Cleaning blue).
    Terminology: "Technician", "Visit", "Asset".
    """
    buffer = BytesIO()
    ...
    locations = report_data.get("locations", [])
    top_reasons = report_data.get("top_sla_reasons", [])

    # --- Header ---
    title_para = Paragraph(
        f"{company.name} — {frequency} Maintenance Report",
        title_style,
    )
    subtitle_para = Paragraph("Generated by MaintainProof", subtitle_style)
    period_para = Paragraph(f"Period: {period_from} to {period_to}", small_style)
    ...
    # --- KPI Cards ---
    visits_count = report_data.get("visits_count", 0) or 0
    violations_count = report_data.get("violations_count", 0) or 0
    issue_rate = report_data.get("issue_rate", 0.0) or 0.0
    success_rate = (1.0 - issue_rate) * 100
    ...
    def _make_kpi_card(value: str, label: str):
        return [
            Paragraph(value, kpi_value_style),
            Spacer(1, 2),
            Paragraph(label, kpi_label_style),
    ...
    if success_rate >= 90:
        ...
        success_color = colors.HexColor("#f59e0b")
    else:
        success_color = colors.HexColor("#ef4444")

    card_width = doc.width / 3 - 4 * mm

    kpi_data = [
        [
            _make_kpi_card(str(visits_count), "Total Visits"),
            _make_kpi_card(str(violations_count), "SLA Violations"),
            _make_kpi_card(f"{success_rate:.0f}%", "Success Rate"),
        ]
    ]
    ...
    # Slate/gray color scheme for maintenance
    kpi_table.setStyle(
        TableStyle(
            [
                ("BACKGROUND", (0, 0), (0, 0), colors.HexColor("#f1f5f9")),  # Slate-100
                ("BACKGROUND", (1, 0), (1, 0), colors.HexColor("#fef2f2")),  # Red-50
                ("BACKGROUND", (2, 0), (2, 0), colors.HexColor("#f0fdf4")),  # Green-50
                ("BOX", (0, 0), (0, 0), 1, colors.HexColor("#64748b")),  # Slate-500
                ("BOX", (1, 0), (1, 0), 1, colors.HexColor("#ef4444")),  # Red
    ...
    # --- Summary text ---
    visits_word = "visit was" if visits_count == 1 else "visits were"
    violations_word = "violation was" if violations_count == 1 else "violations were"

    summary_text = (
        f"In this period, {visits_count} service {visits_word} completed. "
        f"{violations_count} SLA {violations_word} detected ({issue_rate * 100:.1f}% issue rate)."
    )
    ...
    if top_reasons:
        reason_labels = [_get_reason_label(r["code"]) for r in top_reasons if r.get("code")]
        if reason_labels:
            reason_names = ", ".join(reason_labels[:3])
            summary_text += f" Most common issues: {reason_names}."

    if assets:
        main_asset = assets[0]
        asset_name = main_asset.get("name") or ""
        if asset_name and main_asset.get("violations", 0) > 0:
            summary_text += f" Most affected asset: {asset_name}."

    story.append(Paragraph(summary_text, small_style))
    story.append(Spacer(1, 12))

    # --- Technicians table ---
    story.append(Paragraph("Technicians Performance", section_title_style))
    story.append(
        Paragraph(
            "Service visits and SLA violations by technician.",
            small_style,
        )
    )
    story.append(Spacer(1, 4))

    tech_rows = [["Technician", "Visits", "SLA Violations"]]

    for t in technicians:
        name = t.get("name") or "—"
        v = t.get("visits", 0) or 0
        viol = t.get("violations", 0) or 0
        if v == 0 and viol == 0:
            continue
        tech_rows.append([name, v, viol])
    ...
    if len(tech_rows) > 1:
        ...
        story.append(Paragraph("No technician data for this period.", small_style))

    story.append(Spacer(1, 12))

    # --- Assets table ---
    story.append(Paragraph("Assets Service Summary", section_title_style))
    story.append(
        Paragraph(
            "Service visits and issues by asset.",
            small_style,
        )
    )
    story.append(Spacer(1, 4))

    asset_rows = [["Asset", "Type", "Visits", "Violations"]]

    for a in assets[:10]:  # Limit to top 10
        name = a.get("name") or "—"
        type_name = a.get("type_name") or "—"
        v = a.get("visits", 0) or 0
        viol = a.get("violations", 0) or 0
        if v == 0 and viol == 0:
            continue
        asset_rows.append([name, type_name, v, viol])

    if len(asset_rows) > 1:
        asset_table = Table(
            asset_rows,
            colWidths=[doc.width * 0.4, doc.width * 0.25, doc.width * 0.15, doc.width * 0.2],
            hAlign="LEFT",
        )
        ...
        story.append(Paragraph("No asset data for this period.", small_style))

    story.append(Spacer(1, 12))

    # --- Locations table ---
    story.append(Paragraph("Locations Summary", section_title_style))
    story.append(
        Paragraph(
            "Service visits and issues by location.",
            small_style,
        )
    )
    story.append(Spacer(1, 4))

    loc_rows = [["Location", "Visits", "SLA Violations"]]

    for loc in locations:
        name = loc.get("name") or "—"
        v = loc.get("visits", 0) or 0
        viol = loc.get("violations", 0) or 0
        if v == 0 and viol == 0:
            continue
        loc_rows.append([name, v, viol])
    ...
    if len(loc_rows) > 1:
        ...
        story.append(Paragraph("No location data for this period.", small_style))

    story.append(Spacer(1, 12))

    # --- Top SLA reasons ---
    story.append(Paragraph("Top SLA Issues", section_title_style))
    story.append(
        Paragraph(
            "Most frequent causes of SLA violations.",
            small_style,
        )
    )
    story.append(Spacer(1, 4))
    ...
    def _on_page(canvas, doc_):
        _draw_maintenance_report_footer(canvas, doc_, company_name)
...
def _draw_maintenance_report_footer(canvas, doc, company_name: str):
    """
    Footer for maintenance reports.
    """
    canvas.saveState()
    font_name = "Helvetica"
    font_size = 8
    ...
    generated_at = timezone.localtime(timezone.now()).strftime("%Y-%m-%d %H:%M:%S")
    left_text = f"Generated: {generated_at} | {company_name} | MaintainProof"
    right_text = f"Page {doc.page}"
