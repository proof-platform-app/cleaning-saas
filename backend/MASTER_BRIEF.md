...
## 2. Marketing Entry: принципы и архитектура
...
Both pages are non-authenticated, non-interactive, and intentionally separated from the core product UI. Their purpose is to prepare prospects for a guided or live demo, not to replace it. The core message remains unchanged:
...
### Физическое доказательство = физическая точность
...
Short-term revenue goal is defined as reaching **$1,000–$2,000 MRR** through a small number of high-intent customers rather than scale.
...
## 5. Layer 2 — Manager experience / Settings
...
## 6. Архитектура Backend (зафиксировано)
...
* Новая логика → только в `apps.api`
* **Passing tests = истина**

---

## 7. Что реализовано (DONE) — Backend & Core flows
...
## 8. Checklist system — DONE (MVP scope)
...
## 9. Photos — Mobile + Backend (Phase 9) ✅
...
* Check-out с мобильного пока привязан только к статусу job и наличию обоих фото;
  дальнейшие бизнес-правила будем уточнять отдельно.

### Фото: нормализация формата ✅

Нормализация закрыла “разные телефоны → разные PDF/WEB” проблему:
...
## 11. Manager: Planning + Create Job

### Manager Job Planning — backend + frontend DONE

(read-only идеал + create job)
...
## 12. Mobile Cleaner App — текущий статус
...
## 13. Manager Portal (dubai-control) — зафиксировано
...
## 14. Job Planning (Manager) — текущий факт (обновлено)
...
## 15. Работа с Lovable (ЗАФИКСИРОВАНО)
...
## 16. Что делать в новом чате (TODO)
...
## 18. Как стартовать новый чат (обязательный шаблон)
...
## 19. Последние изменения — Web / Admin (Job Planning)
...
## 20. Mobile / Cleaner App — текущее состояние (расширено)
...
## 21. Locations — принципиальное решение
...
## 22. Manager Meta (Create Job prerequisites)
...
## 23. Pricing & Upgrade Flow (Pre-billing)

Pricing-страница и Upgrade-флоу интегрированы с реальным состоянием компании через `usage-summary`.
UI различает анонимных пользователей, активный trial и истёкший trial, адаптируя тексты и CTA.

После истечения trial Standard-план больше не предлагает повторный trial и используется как точка входа в коммерческий диалог (до подключения биллинга).

Текущий Upgrade-флоу является временным и архитектурно подготовлен к интеграции Stripe / Paddle без изменения backend-контрактов.

---

## 24. Фактический статус продукта

CleanProof достиг состояния устойчивого операционного продукта без биллинга:

* backend-ядро job execution закрыто,
* Manager Portal покрывает Planning, Create Job, Job Details и Job History,
* реализован self-serve signup с полноценным trial и реальными backend-ограничениями.

Trial-UX честный и синхронизирован с backend-статусом, без фейковых checkout или скрытых условий.

Архитектура готова к подключению биллинга, но осознанно работает в режиме trial + contact-upgrade.

Проект вышел за рамки прототипа и стабилен для демо, пилотов и ранних пользователей.

---

## 25. SLA / Quality Control (micro-SLA v1)
...
В проект добавлен micro-SLA v1, ориентированный не на контроль или наказание, а на разбор фактов. SLA теперь состоит из трёх уровней: **факт** (`sla_status`), **причина** (`sla_reasons`) и прямой переход к доказательству (**Evidence Jump**).

Это превращает Job History и Job Details в инструмент аудита и разрешения споров: менеджер видит проблему, понимает причину и сразу переходит к конкретному доказательству, не покидая текущий контекст.

Архитектурно SLA реализован без усложнения системы и готов к дальнейшему расширению на следующих слоях.

---

## 26. Performance Layer — управленческий контроль качества
...
## 27. Checklist templates в Job Planning
...
## 29. Job reporting & email delivery (v1)
...
## 31. Reports v2 — Email Delivery & Audit (Completed)
...
## 32. Reports → Evidence layer is completed
...
## 34. Job PDF как доказательная карточка
...
## 35. Job PDF & Email Proof (v1 завершён)
...
## 36. Reporting & Management UX — Status
...
## 37. Email history — manager-facing audit trail

Раздел Email history оформлен как полноценный управленческий инструмент, а не технический лог. Менеджер видит единую историю всех отправленных отчётов с возможностью быстро отфильтровать: дату отправки, тип отчёта (job, weekly, monthly), конкретный job или период, получателя и статус доставки (`sent / failed`).

Фильтрация по датам работает строго по фактической дате отправки email, что исключает путаницу между датой job и датой формирования отчёта.

Экран готов к дальнейшему развитию (повторная отправка, SLA доставки, диагностика ошибок).

---

## 38. Commercial enforcement & read-only mode (DONE)
...
## 39. Cleaner access & security model

Доступ клинеров к мобильному приложению построен по принципу **manager-controlled access**.

* Клинеры не регистрируются самостоятельно и не имеют механизма восстановления PIN.
* Менеджер является единственной точкой управления: он создаёт клинера, активирует/деактивирует его и при необходимости сбрасывает PIN.

Такая модель упрощает онбординг полевых сотрудников, подходит для рынков с низким использованием email и соответствует требованиям MVP по скорости и надёжности.

---

## 40. Force-complete / override (manager-only)
...
## 41. SLA philosophy: proof-first, violations as exceptions
...
## 42. Force complete — controlled exception handling
...
## 43. SLA модель и принудительное завершение джоба
...
---
**SLA Engine v2 (analytics слой)**

Поверх базового SLA-движка реализован аналитический слой:
менеджер может видеть не только факт нарушений, но и их структуру:

- топ причин (late start, checklist_not_completed, proof_missing и т.п.);
- топ клинеров по нарушениям;
- топ локаций, где чаще всего ломается SLA.

Это создаёт базу для следующего шага:
автоматические алерты, скоринг клинеров и SLA-профили под разных клиентов.

## 44. Analytics — ключевой управленческий слой

**Analytics Page** зафиксирована как отдельный управленческий слой, а не часть Reports.
...
Если захочешь, дальше могу сделать из этого файла «режимы» — короткий top-level summary, версию для инвестора и техническую выжимку для разработчиков, но уже на базе этой структурированной версии.

**. Analytics — ключевой управленческий слой**
добавить **один абзац в конец**:

---

**Analytics v1 — статус**

Analytics Page подключена к реальным backend-данным и перестала быть mock-слоем.
Менеджер видит живые KPI, тренды и performance клинеров за выбранный период.
Этот слой зафиксирован как основа для SLA Engine v2, performance scoring и автоматических управленческих алертов.
---

### SLA Engine v2 — фактическое состояние

На уровне бэкенда реализован базовый SLA-движок:

* для каждой `Job` рассчитывается:
  * `sla_status` — `ok` или `violated`;
  * набор `sla_reasons` (коды: `late_start`, `missing_after_photo`,
    `checklist_not_completed` и т.д.), через helper
    `compute_sla_status_and_reasons_for_job(job)`;
* поверх этого слоя добавлен аналитический эндпоинт:

  * `GET /api/manager/analytics/sla-breakdown/` — агрегаты по:
    * доле нарушенных job (`violation_rate`);
    * причинам нарушений (`reasons`);
    * клинерам (`top_cleaners`);
    * локациям (`top_locations`).

На фронте блок **SLA Performance** на странице Analytics использует именно этот
эндпоинт и не дублирует логику SLA в JavaScript.

Дальнейшее развитие SLA (детализация причин, тренды, чек-листы, Reports)
должно опираться на этот слой как на **единственный source of truth**.
> **SLA Engine v2**
>
> В ядре backend реализован единый SLA-движок, который рассчитывает статус SLA и причины нарушений прямо на уровне `Job`.
>
> * Helper `compute_sla_status_and_reasons_for_job(job)` нормализует explicit-override (`job.sla_reasons`) и, при его отсутствии, сам определяет нарушения по факту выполненного proof: наличие before/after-фото и закрытых обязательных пунктов чек-листа.
> * На этой функции построены:
>
>   * `_get_company_report()` — единый расчёт weekly/monthly SLA-отчётов (jobs, violations, issue_rate, топы по клинерам/локациям и top-reasons);
>   * `OwnerOverviewView` — управленческая сводка по SLA за период;
>   * `ManagerPerformanceView` — агрегаты по клинерам и локациям с violation-rate и флагом повторяющихся нарушений;
>   * `ManagerViolationJobsView` — выборка всех конкретных jobs по заданному типу нарушения (drill-down).
> * Force-complete (`ManagerJobForceCompleteView`) интегрирован в SLA Engine: менеджер может явно завершить job, указав причину (`reason_code`) и комментарий; эта причина попадает в `job.sla_reasons` и далее в аналитику/отчёты.
> * Все weekly/monthly отчёты (JSON, PDF и e-mail) считаются из одного и того же SLA-слоя, без дублирования логики и с едиными правилами тайминга (только `actual_end_time.date`, только `completed` jobs).

### Jobs and Job History — product separation

The platform separates **operational job management** from **historical analysis**.

- **Jobs** is an operational workspace for managers:
  - Today, Upcoming, and recently Completed jobs (last 30 days by default).
  - Optimized for fast daily decisions and monitoring.

- **Job History** is the full historical archive:
  - unlimited date ranges,
  - filtering by status, proof completeness, and issues,
  - used for audits, reporting, and retrospective analysis.

This separation keeps the Jobs interface clean and responsive while preserving
full data access through Job History.

**Reports philosophy (Owner vs Manager)**

Reports are not analytics dashboards.

Their purpose is to answer:

* Owners: *“Is the business under control?”*
* Managers: *“Why are there issues and what should I fix?”*

Owner view principles:

* One screen.
* One glance.
* One conclusion.
* Issue rate is the primary signal, reinforced with a qualitative status (Healthy / Needs attention / At risk).
* No operational details, no drill-down.

Manager view principles:

* Builds on the owner summary.
* Explains causes (SLA reasons).
* Identifies responsibility (cleaners, locations).
* Leads naturally to corrective actions via violation lists.

Reports intentionally avoid:

* Trend charts and timelines.
* Deep filtering and exploratory analytics.
* Comparisons between periods (WoW / MoM).

Those belong to Analytics, not Reports.

---
## Reporting model

Reports — это управленческий слой, а не аналитика и не Job History.
Owner видит один экран, один период (rolling 30 days), один вывод о состоянии бизнеса.
Manager видит ту же картину + причины нарушений и точки действия.
Reports не содержат таймлайнов, сравнений периодов и сложных фильтров — только narrative summary и accountability.

Email as audit trail
Email history — часть Reports.
UI, PDF и email используют один и тот же источник данных, расхождений быть не должно.

### ## SLA Reports — Detailed Violations View

В менеджерских SLA-отчётах реализован полный переход от агрегированных показателей к конкретным задачам.
...
используются как entry-point’ы для перехода к детализированному списку job’ов, нарушивших SLA за выбранный период.

### ## Violation Jobs Page

Страница детальных нарушений:

* отображает активные фильтры (причина, клинер, локация)
* фиксирует период отчёта
* визуализирует статус job’ов бейджами
* поддерживает быстрый просмотр job’а
* поддерживает переход на полную страницу job’а

Обеспечена сквозная трассировка:

**SLA-метрика → причина → конкретная задача**

---
## Analytics & SLA (completed milestone)
Реализован полноценный аналитический слой CleanProof: summary-карточки, тренды, SLA Performance, violation reasons и performance breakdown по клинерам и локациям. Архитектурно analytics работает как read-only слой поверх execution-ядра и SLA engine, без дублирования логики во frontend. Это превращает систему из job-трекера в управленческий инструмент: менеджер видит не только факт выполнения, но и качество, стабильность и причины нарушений.
