  ...
  return (
    <div className="rounded-xl border border-border bg-card shadow-card">
      <div className="border-b border-border p-6">
        <h3 className="text-base font-semibold text-foreground">Cleaner Performance</h3>
        <p className="text-sm text-muted-foreground">Weekly performance metrics by cleaner</p>
      </div>
                ...
                >
                  <td className="whitespace-nowrap px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {isTopPerformer && (
                          <span className="text-xs text-analytics-success">Top Performer</span>
                        )}
                      </div>
                    </div>
                  </td>
                  <td className="whitespace-nowrap px-6 py-4 text-center">
                    <span className="font-semibold text-foreground">{cleaner.jobsCompleted}</span>
                  </td>
                  <td className="whitespace-nowrap px-6 py-4 text-center">
                    <div className="flex items-center justify-center gap-1.5">
                      {slowDuration && <Clock className="h-3.5 w-3.5 text-analytics-warning" />}
                      <span className={cn(
                        'font-medium',
                        slowDuration ? 'text-analytics-warning' : 'text-foreground'
                      )}>
                  ...
                  <td className="whitespace-nowrap px-6 py-4 text-center">
                    <span className={cn(
                      ...
                      {cleaner.onTimeRate >= 95 && <CheckCircle2 className="h-3 w-3" />}
                      {cleaner.onTimeRate}%
                    </span>
                  </td>
                  <td className="whitespace-nowrap px-6 py-4 text-center">
                    <div className="flex items-center justify-center">
                      <div className="h-1.5 w-16 overflow-hidden rounded-full bg-muted">
                        <div 
                          ...
                          style={{ width: `${cleaner.proofCompletionRate}%` }}
                        />
                      </div>
                  ...
                  <td className="whitespace-nowrap px-6 py-4 text-center">
                    {cleaner.issuesCount === 0 ? (
                      ...
                      <span className={cn(
                        ...
                        hasIssues ? 'text-analytics-danger' : 'text-analytics-warning'
                      )}>
                        {hasIssues && <AlertTriangle className="h-3.5 w-3.5" />}
                        {cleaner.issuesCount}
                      </span>
                    )}
                  </td>
